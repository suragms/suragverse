  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto text-center">
        ...
        {/* Cinematic portal card — Antigravity doorway */}
        <motion.a
          ...
          className="group relative block max-w-3xl mx-auto rounded-[2rem] overflow-hidden border border-glass-border text-left shadow-[0_20px_60px_rgba(0,0,0,0.5)] cursor-pointer"
          aria-label={`Open my portfolio at ${contactInfo.portfolioLabel}`}
        >
          {/* Background image with depth movement */}
          <div className="relative aspect-[16/9] sm:aspect-[2/1] overflow-hidden">
            <motion.div style={{ y: bgY }} className="absolute -inset-6">
              <Image
                ...
                loading="lazy"
                quality={80}
                className="object-cover object-center transition-transform duration-[1.8s] ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-105"
            ...
            {/* Content floating into position */}
            <motion.div
              style={{ y: contentY }}
              className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 z-10"
            >
              <Sparkles size={20} className="text-electric-blue mb-4 animate-pulse" />
              <p className="font-[family-name:var(--font-heading)] text-lg sm:text-2xl font-bold tracking-tight leading-tight mb-6 max-w-lg">
                DISCOVER THE
                <br />
                DETAILED WORK.
              </p>
              <span className="inline-flex items-center gap-3 px-7 sm:px-9 py-3.5 sm:py-4 rounded-full bg-white !text-black text-xs sm:text-sm font-bold tracking-wider group-hover:bg-electric-blue group-hover:!text-black transition-colors duration-500 min-h-[48px] shadow-lg btn-antigravity">
                ENTER MY PORTFOLIO
                <ArrowUpRight
                  size={17}
